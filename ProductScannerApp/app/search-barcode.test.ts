...
describe('SearchBarcode', () => {
  ...
  it('returns product data', async () => {
    const fetch = jest.fn<any, any>(() => Promise.resolve({ data: simple.testHttpResponse }))
    const product = await createSearchBarcode({ fetch, ...otherDeps })(simple.testBarcode)
    expect(product).toEqual(simple.testResult)
  })

  it('aggregates product data', async () => {
    const fetch = jest.fn<any, any>(() => Promise.resolve({ data: aggregate.testHttpResponse }))
    const product = await createSearchBarcode({ fetch, ...otherDeps })(aggregate.testBarcode)
  ...
  it('errors when query fails', async () => {
    ...
    await expect(createSearchBarcode({ fetch, ...otherDeps })(simple.testBarcode)).rejects.toThrow(SearchBarcodeError)
  });
})
